...
function Home() {
  const [labList, setLabList] = useState(labs.status ? labs.labs : []);
  ...
  return (
    <div className="w-screen h-screen pt-32 flex flex-col items-center">
      ...
      {labList.length > 0 ? (
        ...
        <h1 className="text-2xl">
          Não há{" "}
          <span className="font-bold text-iflab_green_light">laboratórios</span>{" "}
        </h1>
      ) : (
        <h1 className="text-2xl">
          Você ainda não pertence a nenhum{" "}
          <span className="font-bold text-iflab_green_light">laboratório</span>
        </h1>
      )}
      ...
      <div className="h-fit max-h-full w-fit overflow-y-auto p-10 mt-10 gap-10 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {labList.map((lab) => (
          <LabCard key={lab.labId} labId={lab.labId} userAccessLevel={lab.userLevel} />
        ))}
        {user.campusAdminLevel >= 2 ? (
          <div className="w-[25rem] h-[15rem] bg-iflab_white_light rounded-lg shadow-md hover:shadow-xl hover:scale-105 flex flex-col justify-center items-center duration-75 cursor-pointer">
